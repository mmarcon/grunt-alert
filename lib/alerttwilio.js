...
module.exports = function(request){
    return function alertSlack(config, grunt, done, logger) {
        var payload = {},
        ...
        if(!config.token || !config.account) {
            ...
            logger.error('Twilio account id or authorization token not set');
            return done();
        }
        ...
        request({
            ...
            if(e) {
                ...
                logger.error('Failed to post to Twilio: ' + JSON.stringify(e));
                return done();
            }
            logger.log('Successfully posted to Twilio <' + config.message + '>');
            return done(body);
        });
    };
};