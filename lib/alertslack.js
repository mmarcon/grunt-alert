...
module.exports = function(request){
    return function alertSlack(config, grunt, done, logger) {
        ...
        if(!url) {
            ...
            logger.error('Webhook URL for Slack is not set');
            return done();
        }
        ...
        request({
            ...
            if(e) {
                ...
                logger.error('Failed to post to slack: ' + JSON.stringify(e));
                return done();
            }
            logger.log('Successfully posted to slack <' + config.message + '>');
            return done(body);
        });
    };
};